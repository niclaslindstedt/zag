...
At the end of every agent turn the session emits a **`turn_complete`** event (Swift case `.turnComplete(TurnCompletePayload)`) carrying the provider's `stopReason` (`end_turn`, `tool_use`, `max_tokens`, `stop_sequence`, or `nil`), a zero-based monotonic `turnIndex`, and the turn's `usage`. A per-turn `result` event fires immediately after. New code should key turn-boundary UI off `turn_complete` — it is the authoritative signal and carries richer metadata than `result`. `result` continues to fire per-turn for backward compatibility.
