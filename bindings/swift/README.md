...
Because Swift `struct` errors cannot subclass, `ZagFeatureUnsupportedError`
is a sibling of `ZagError` rather than a subclass. Catch it directly, or
catch generic `Error`:

```swift
do {
    _ = try await ZagBuilder().provider("ollama").addDir("/extra").exec("...")
} catch let err as ZagFeatureUnsupportedError {
    print("pick another provider from: \(err.supportedProviders.joined(separator: ", "))")
} catch {
    // other ZagError / errors
}
```

`ZagFeatureUnsupportedError` exposes `asZagError` for callers that want a
`ZagError` value with the same message.
...
Capability checks only apply to local execution mode.
