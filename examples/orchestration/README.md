...
Shell scripts demonstrating multi-agent orchestration patterns using zag's CLI primitives. Each script is self-contained and showcases one or more patterns from the [orchestration guide](../../zag-agent/man/orchestration.md).
...
Decision Arena (07)             Meta-Bootstrap (08)
┌──────────┐   ┌──────────┐    ┌────────────┐   ┌────────────┐
│ Advocate │   │ Skeptic  │    │--help-agent│──►│Agent writes│──► saved
│ (FOR)    │   │ (AGAINST)│    │ + task desc│   │  script    │   script
└────┬─────┘   └────┬─────┘    └────────────┘   └────────────┘
     │   cross-poll  │
     │◄─────────────►│
     │   rebuttals   │
